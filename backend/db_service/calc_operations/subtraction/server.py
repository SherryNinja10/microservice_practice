...
def subtraction():
    try:
        ...
        if token:
            ...
            cur.execute(
                "INSERT INTO subtraction (number1, number2, answer, user_id) VALUES (%s, %s, %s, %s)", (number1, number2, answer, user_id)
            )

            connection.commit()

            return jsonify({ "answer": answer, "calculation": f"{number1} - {number2}", "message": "Subtraction calculation done and added to subtraction history" }), 200
