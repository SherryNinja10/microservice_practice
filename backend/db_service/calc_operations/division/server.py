...
def division():
    try:
        ...
        if token:
            ...
            cur.execute(
                "INSERT INTO division (number1, number2, answer, user_id) VALUES (%s, %s, %s, %s)", (number1, number2, answer, user_id)
            )

            connection.commit()

            return jsonify({ "answer": answer, "calculation": f"{number1} / {number2}", "message": "Division calculation done and added to division history" }), 200
